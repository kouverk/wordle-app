...
-- Insert all 2,309 NYT Wordle solution words
INSERT INTO `nyt_words` (`word`) VALUES
...
-- Useful queries to run after creating this table:

-- Count how many words overlap
-- SELECT COUNT(*) as overlap_count FROM words w
-- INNER JOIN nyt_words n ON UPPER(w.word) = n.word;

-- Words in YOUR list that are NOT in NYT (the obscure ones)
-- SELECT w.word, w.frequency, w.score FROM words w
-- LEFT JOIN nyt_words n ON UPPER(w.word) = n.word
-- WHERE n.word IS NULL
-- ORDER BY w.frequency DESC
-- LIMIT 100;

-- Words in NYT that are NOT in your list (you're missing these common words)
-- SELECT n.word FROM nyt_words n
-- LEFT JOIN words w ON n.word = UPPER(w.word)
-- WHERE w.word IS NULL;

-- Your words that ARE in NYT list (the good common ones)
-- SELECT w.word, w.frequency, w.score FROM words w
-- INNER JOIN nyt_words n ON UPPER(w.word) = n.word
-- ORDER BY w.frequency DESC;
